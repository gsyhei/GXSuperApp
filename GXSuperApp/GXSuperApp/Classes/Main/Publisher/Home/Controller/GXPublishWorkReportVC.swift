...
class GXPublishWorkReportVC: GXBaseViewController {
    lazy var tableView: GXBaseTableView = {
        return GXBaseTableView(_frame: self.view.bounds, _style: .plain).then {
            ...
            $0.dataSource = self
            $0.delegate = self
            $0.placeholder = "暂无工作汇报"
            $0.setAddButton(title: "添加工作汇报", type: 0) {[weak self] in
                guard let `self` = self else { return }
                self.rightButtonItemTapped()
            }
            $0.register(cellType: GXPublishWorkReportCell.self)
        }
    }()
    ...
    private lazy var viewModel: GXPublishWorkReportViewModel = {
        return GXPublishWorkReportViewModel()
    }()
...
extension GXPublishWorkReportVC {
    func requestData(isRefresh: Bool, isShowHud: Bool, completion: ((Bool, Bool) -> (Void))? = nil) {
        if isShowHud {
            MBProgressHUD.showLoading(to: self.view)
        }
        self.viewModel.requestGetActivityReportInfo(refresh: isRefresh, success: {[weak self] isLastPage in
...
extension GXPublishWorkReportVC {
    ...
    @objc func rightButtonItemTapped() {
        let vc = GXPublishWorkReportEditVC.createVC(activityId: self.viewModel.activityId)
        self.navigationController?.pushViewController(vc, animated: true)
    }

}

extension GXPublishWorkReportVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: GXPublishWorkReportCell = tableView.dequeueReusableCell(for: indexPath)
        ...
        cell.avatarAction = {[weak self] curCell in
            ...
            guard let userId = self.viewModel.list[curIndexPath.row].creatorId else { return }
