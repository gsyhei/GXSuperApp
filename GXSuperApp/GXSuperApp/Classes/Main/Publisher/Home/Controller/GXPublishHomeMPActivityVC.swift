//
//  GXPublishHomeMPActivityVC.swift
//  GXHeiVibe
//
...
class GXPublishHomeMPActivityVC: GXBaseViewController {
    ...
    private lazy var viewModel: GXPublishHomeMPActivityViewModel = {
        return GXPublishHomeMPActivityViewModel()
    }()
...
extension GXPublishHomeMPActivityVC {
...
extension GXPublishHomeMPActivityVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc.deleteAction = {[weak self] activityId in
            guard let `self` = self else { return }
            self.viewModel.list.removeAll(where: { $0.id == activityId })
            self.tableView.gx_reloadData()
        }
        self.navigationController?.pushViewController(vc, animated: true)
    }
