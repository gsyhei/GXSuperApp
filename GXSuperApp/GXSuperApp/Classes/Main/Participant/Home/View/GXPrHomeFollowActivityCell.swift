//
//  GXPrHomeFollowActivityCell.swift
//  GXHeiVibe
//
...
class GXPrHomeFollowActivityCell: UITableViewCell, NibReusable {
...
//    @IBOutlet weak var tagsViewWLC: NSLayoutConstraint!

    var avatarAction: GXActionBlockItem<GXPrHomeFollowActivityCell>?
    ...
    func bindCell(model: GXActivityBaseInfoData?) {
        guard let data = model else { return }

        self.avatarButton.kf.setImage(with: URL(string: data.avatarPic), for: .normal, placeholder: UIImage.gx_defaultAvatar)
        self.nicknameLabel.text = data.nickName
//        let tagsWidth = self.tagsView.updateTags(isVip: data.vipFlag, 
//                                                 isJg: data.orgAccreditationFlag,
//                                                 isGf: data.officialFlag,
//                                                 isDr: data.expertFlag)
//        self.tagsViewWLC.constant = tagsWidth
        ...
        if let date = Date.date(dateString: data.approveTime, format: "yyyy-MM-dd HH:mm:ss") {
            self.createTimeLabel.text = "发布时间：" + date.string(format: "yyyy.MM.dd HH:mm")
        } else if let date = Date.date(dateString: data.createTime, format: "yyyy-MM-dd HH:mm:ss") {
            self.createTimeLabel.text = "发布时间：" + date.string(format: "yyyy.MM.dd HH:mm")
        } else {
            self.createTimeLabel.text = "发布时间：" + data.approveTime
        }
...
extension GXPrHomeFollowActivityCell {
