...
class GXMessagesLeftCell: UITableViewCell, NibReusable {
    ...
    var avatarAction: GXActionBlockItem<GXMessagesLeftCell>?
    var imageAction: GXActionBlockItem<GXMessagesLeftCell>?
    ...
    func bindCell(model: GXListUserMessagesItem?, staffsUserIds: Set<Int>) {
        ...
        if let data = Date.date(dateString: item.updateTime, format: "yyyy-MM-dd HH:mm:ss") {
            self.dateLabel.text = data.string(format: "yyyy.MM.dd HH:mm")
        } else {
        ...
        if staffsUserIds.first(where: { $0 == item.userId }) == nil {
            self.staffsIView.isHidden = true
        }
        else {
            self.staffsIView.isHidden = false
        }
    }
}

extension GXMessagesLeftCell {
