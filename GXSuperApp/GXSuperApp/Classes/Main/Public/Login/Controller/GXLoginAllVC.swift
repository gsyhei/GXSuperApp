...
class GXLoginAllVC: GXBaseViewController {
    ...
    @IBOutlet weak var wexinLoginButton: UIButton!
    @IBOutlet weak var appleLoginButton: UIButton!
    @IBOutlet weak var appleCenterLC: NSLayoutConstraint!
    ...
    override func setupViewController() {
        ...
        else if GXUserManager.shared.roleType == .participant {
            self.gx_addBackBarButtonItem()
        }
        if self.loginType == .login {
            self.titleLabel.text = "登录/注册"
            self.otherInfoLabel.isHidden = false
            self.wexinLoginButton.isHidden = false
            ...
            if WXApi.isWXAppInstalled() && WXApi.isWXAppSupport() {
                self.appleCenterLC.constant = 32.0
                self.wexinLoginButton.isHidden = false
            }
            else {
                self.appleCenterLC.constant = 0.0
                self.wexinLoginButton.isHidden = true
            }
        }
        else {
            self.titleLabel.text = "绑定手机号"
            self.otherInfoLabel.isHidden = true
            self.wexinLoginButton.isHidden = true
    ...
    @IBAction func wexinLoginButtonClicked(_ sender: UIButton) {
        GXLoginManager.shared.wexinLogin()
    }

    @IBAction func appleLoginButtonClicked(_ sender: UIButton) {
        GXLoginManager.shared.appleLogin()
    }
    
    @objc public override func gx_backBarButtonItemTapped() {
...
extension GXLoginAllVC: UITextViewDelegate {
    ...
    func didLinkScheme(_ scheme: String) {
        ...
        case "yhxy":
            ...
            let vc = GXWebViewController(urlString: url, title: "用户协议")
            self.navigationController?.pushViewController(vc, animated: true)
        case "yszc":
            let url = Api_WebBaseUrl + "/h5/#/agreement/6"
            let vc = GXWebViewController(urlString: url, title: "隐私协议")
            self.navigationController?.pushViewController(vc, animated: true)
        default :
            XCGLogger.debug("这个是普通的url")
        }
    }
}
